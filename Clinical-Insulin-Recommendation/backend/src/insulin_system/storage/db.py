...
def _apply_sqlite_pragmas(conn) -> None:
    """Per-connection settings: WAL durability, fewer lock errors, referential checks."""
...
def _ensure_performance_indexes(conn) -> None:
    """Speed up patient-scoped reads (monitoring, assessments, alerts)."""
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_glucose_patient_reading ON glucose_readings(patient_id, reading_at)"
    )
    conn.execute("CREATE INDEX IF NOT EXISTS idx_records_patient_created ON records(patient_id, created_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_dose_patient_created ON dose_events(patient_id, created_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_alerts_patient_resolved ON alerts(patient_id, resolved)")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_smart_sensor_patient ON smart_sensor_predictions(patient_id)"
    )


def _conn_for_file(db_file: Path):
    """Open the DB with shared pragmas; ensures schema exists via init_db."""
    import sqlite3

    init_db(db_file.parent if db_file else None)
    conn = sqlite3.connect(str(db_file), timeout=10.0)
    _apply_sqlite_pragmas(conn)
    return conn


def check_database_integrity(db_path: Optional[Path] = None) -> Dict[str, Any]:
    """Run SQLite integrity_check (fast validation; use after backup/restore)."""
    path = get_db_path(db_path)
    if not path.exists():
        return {"ok": True, "detail": "no database file yet"}
    conn = _conn_for_file(path)
    try:
        row = conn.execute("PRAGMA integrity_check").fetchone()
        ok = row and row[0] == "ok"
        return {"ok": ok, "detail": row[0] if row else "unknown"}
    finally:
        conn.close()
...
def mark_notification_read(notification_id: int, db_path: Optional[Path] = None) -> bool:
    ...
    conn = _conn_for_file(path)
    try:
        cur = conn.execute("UPDATE notifications SET unread = 0 WHERE id = ?", (int(notification_id),))
        conn.commit()
...
def delete_notification(notification_id: int, db_path: Optional[Path] = None) -> bool:
    ...
    conn = _conn_for_file(path)
    try:
        cur = conn.execute("DELETE FROM notifications WHERE id = ?", (int(notification_id),))
        conn.commit()
        return cur.rowcount > 0
    finally:
        conn.close()
...
def insert_alert(
    ...
    try:
        cur = conn.execute(
            "INSERT INTO alerts (created_at, severity, title, text, resolved, patient_id) VALUES (?, ?, ?, ?, 0, ?)",
            (datetime.now(timezone.utc).isoformat(), severity[:20], title[:200], text[:2000], patient_id),
        )
        conn.commit()
        return cur.lastrowid
    finally:
...
def delete_patient_monitoring_rows(patient_id: int, db_path: Optional[Path] = None) -> None:
    """Remove glucose, assessment records, dose events, smart-sensor rows, and patient-linked alerts for re-seeding demos."""
    path = get_db_path(db_path)
    if not path.exists():
        return
    conn = _conn_for_file(path)
    try:
        conn.execute("DELETE FROM glucose_readings WHERE patient_id = ?", (patient_id,))
        conn.execute("DELETE FROM records WHERE patient_id = ?", (patient_id,))
        conn.execute("DELETE FROM dose_events WHERE patient_id = ?", (patient_id,))
        try:
            conn.execute("DELETE FROM smart_sensor_predictions WHERE patient_id = ?", (patient_id,))
        except Exception:
            pass
        try:
...
def insert_glucose_reading(
    ...
    conn = _conn_for_file(path)
    try:
        ts = reading_at if reading_at else datetime.now(timezone.utc).isoformat()
        cur = conn.execute(
            "INSERT INTO glucose_readings (reading_at, value, is_predicted, patient_id) VALUES (?, ?, ?, ?)",
            (ts, int(round(float(value))), 1 if is_predicted else 0, patient_id),
        )
        conn.commit()
        return cur.lastrowid
    finally:
...
def insert_dose_event(
    ...
    conn = _conn_for_file(path)
    try:
        ts = created_at if created_at else datetime.now(timezone.utc).isoformat()
        cur = conn.execute(
            """INSERT INTO dose_events (created_at, meal_bolus, correction_dose, total_dose, request_id, patient_id)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (ts, meal_bolus, correction_dose, total_dose, request_id, patient_id),
        )
        conn.commit()
        return cur.lastrowid
    finally:
