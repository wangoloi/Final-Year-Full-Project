...
def save_best_model(
    ...
    # Atomic write: avoids truncated .joblib if the process is killed mid-write (OneDrive/sync can expose this)
    bundle_path = out_dir / BUNDLE_FILENAME
    fd, tmp = tempfile.mkstemp(suffix=".joblib.tmp", dir=str(out_dir))
    os.close(fd)
    try:
...
def load_best_model(
    ...
    if file_size < 1000:
        raise RuntimeError(
            ...
            "Regenerate: python scripts/quick_train_inference_bundle.py"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Model file is corrupted or incomplete. Regenerate: "
            "python scripts/quick_train_inference_bundle.py (from Clinical-Insulin-Recommendation)"
        ) from e
    except Exception as e:
        logger.exception("Failed to load model from %s", bundle_path)
        raise RuntimeError(
            f"Could not load model: {e}. Regenerate: python scripts/quick_train_inference_bundle.py"
        ) from e
