...
def test_register(client):
    ...
    assert data["user"].get("role") == "patient"
...
def test_me(client, test_user, auth_headers):
    ...
    assert r.json()["user"].get("role") == "patient"
...
def test_sensor_demo_forbids_patient(client, auth_headers):
    """Sensor demo is clinician-only."""
    r = client.get("/api/sensor-demo/meta", headers=auth_headers)
    assert r.status_code == 403


def test_sensor_demo_meta_patients_series_summary(client, auth_headers_clinician):
    meta = client.get("/api/sensor-demo/meta", headers=auth_headers_clinician)
    ...
    p = client.get("/api/sensor-demo/patients?limit=5", headers=auth_headers_clinician)
    assert p.status_code == 200
    ids = p.json().get("patients") or []
    assert len(ids) >= 1
    pid = ids[0]

    s = client.get(f"/api/sensor-demo/series?patient_id={pid}&limit=10", headers=auth_headers_clinician)
    assert s.status_code == 200
    readings = s.json().get("readings") or []
    assert len(readings) >= 1
    assert "glucose_level" in readings[0]

    bad = client.get("/api/sensor-demo/series?patient_id=__no_such_patient__", headers=auth_headers_clinician)
    assert bad.status_code == 404

    summ = client.get(f"/api/sensor-demo/summary?patient_id={pid}&last_n=20", headers=auth_headers_clinician)
...
def test_chatbot_breakfast_meal_combo(client, auth_headers, chat_session):
    r = client.post(
        "/api/chatbot/message",
        headers=auth_headers,
        json={"message": "give me a diabetes friendly breakfast", "session_id": chat_session},
    )
    assert r.status_code == 200
    text = r.json()["response"].lower()
    assert "breakfast" in text
    assert "gi " in text or "balanced plate" in text or " + " in text


def test_chatbot_lists_requested_number_of_foods(client, auth_headers, chat_session):
    r = client.post(
        "/api/chatbot/message",
        headers=auth_headers,
        json={"message": "give me 5 diabetes friendly foods", "session_id": chat_session},
    )
    assert r.status_code == 200
    text = r.json()["response"]
    assert "app database" in text.lower()
    numbered = re.findall(r"(?m)^\d+\.\s", text)
    assert len(numbered) >= 5


def test_chatbot_full_day_meal_plan(client, auth_headers, chat_session):
    r = client.post(
        "/api/chatbot/message",
        headers=auth_headers,
        json={"message": "give me a diabetes friendly meal plan for a day", "session_id": chat_session},
    )
    assert r.status_code == 200
    text = r.json()["response"].lower()
    assert "breakfast" in text
...
def test_chatbot_vegetarian_breakfast_personalization(client, auth_headers, chat_session):
    r = client.post(
        "/api/chatbot/message",
        headers=auth_headers,
        json={"message": "give me a vegetarian diabetes friendly breakfast", "session_id": chat_session},
    )
    assert r.status_code == 200
    text = r.json()["response"].lower()
    assert "vegetarian" in text
    assert "chicken" not in text
    assert "tilapia" not in text


def test_chatbot_local_food_list_includes_local_name(client, auth_headers, chat_session):
    r = client.post(
        "/api/chatbot/message",
        headers=auth_headers,
        json={"message": "give me 3 local ugandan diabetes friendly foods", "session_id": chat_session},
    )
    assert r.status_code == 200
    text = r.json()["response"].lower()
    assert "local ugandan" in text
    assert "local name:" in text


def test_chatbot_high_reading_breakfast_context(client, auth_headers, chat_session):
    r = client.post(
        "/api/chatbot/message",
        headers=auth_headers,
        json={"message": "my sugar is high, suggest a diabetes friendly breakfast", "session_id": chat_session},
    )
    assert r.status_code == 200
    text = r.json()["response"].lower()
    assert "care plan" in text or "follow your care plan" in text or "reading is high" in text
    assert "breakfast" in text
